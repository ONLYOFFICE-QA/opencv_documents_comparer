...
from frameworks.microsoft_office import Excel
...
class OpenerXlsx(Excel):
    EXCEPTION_FILES = FileUtils.read_json(f"{getcwd()}/assets/exception_file.json")
    ...
    def run_opener(self, list_of_files):
        ...
        for self.doc_helper.converted_file in list_of_files:
            if not self.doc_helper.converted_file.lower().endswith(".xlsx"):
                continue
            if self.doc_helper.converted_file in self.EXCEPTION_FILES["xlsx_files_opening_error_named_range"]:
