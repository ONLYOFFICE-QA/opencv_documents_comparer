...
from frameworks.host_control.key_actions import KeyActions
...
class Excel:
            ...
            if self.errors:
                match self.errors:
                    case ['#32770', 'Microsoft Visual Basic']:
                        if not KeyActions.click(f"{StaticData.PROJECT_DIR}/assets/image_templates/excel/end.png"):
    ...
    def prepare_excel_for_test(self):
        if KeyActions.click(join(StaticData.PROJECT_DIR, 'assets', 'image_templates', '', 'turn_on_content.png')):
            sleep(1)
            win32gui.EnumWindows(self.get_windows_title, self.errors)
            if self.errors:
                self.errors.clear()
